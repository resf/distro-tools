...
async def admin_supported_products(
    ...
    for product in products.items:
        mirrors_count = await SupportedProductsRhMirror.filter(
...
async def export_all_configs(
    ...
    mirrors = await query.prefetch_related("supported_product", "rpm_repomds").all()
    ...
    for mirror in mirrors:
        ...
        if production_only is not None:
            mirror_data["repositories"] = [
                repo
                for repo in mirror_data["repositories"]
            ...
            for j, repo in enumerate(repositories):
                ...
                for key in repo_required:
                    if key not in repo or repo[key] is None:
                        errors.append(
                            f"Config {i+1}, Repo {j+1}: Missing required field '{key}'"
                        )
...
async def _import_configuration(
    import_data: List[Dict[str, Any]], replace_existing: bool = False
) -> Dict[str, Any]:
    ...
    for config in import_data:
        ...
        if existing_mirror and replace_existing:
            ...
            await SupportedProductsRpmRepomd.filter(
                supported_products_rh_mirror=existing_mirror
            ).delete()
...
async def import_configurations(
    request: Request, file: UploadFile = File(...), replace_existing: bool = Form(False)
):
    ...
    try:
        ...
        return RedirectResponse(
            f"/admin/supported-products?success={success_message}", status_code=302
        )
    except Exception as e:
        return templates.TemplateResponse(
            "admin_supported_products.jinja",
...
async def admin_supported_product(request: Request, product_id: int):
    product = await SupportedProduct.get_or_none(id=product_id).prefetch_related(
        "rh_mirrors", "rh_mirrors__rpm_repomds", "code"
    )
    if product is None:
        return templates.TemplateResponse(
            "error.jinja",
...
async def admin_supported_product_delete(request: Request, product_id: int):
    ...
    affected_products_count = await AdvisoryAffectedProduct.filter(
        supported_product=product
    ).count()
    ...
    if mirrors_count > 0 or packages_count > 0 or affected_products_count > 0:
        ...
        error_message = (
            f"Cannot delete supported product '{product.name}' because it has associated "
    ...
    return RedirectResponse(
        "/admin/supported-products?success=Supported product deleted successfully",
        status_code=302,
    )
...
async def admin_supported_product_mirror(
    request: Request, product_id: int, mirror_id: int
):
...
async def admin_supported_product_mirror_delete(
    ...
    if blocks_count > 0 or overrides_count > 0:
        ...
        error_message = (
            f"Cannot delete mirror '{mirror.name}' because it has associated "
...
async def admin_supported_product_mirrors_bulk_delete(
    ...
    """Bulk delete multiple mirrors with dependency checking"""
    if not mirror_ids or mirror_ids.strip() == "":
        return RedirectResponse(
            f"/admin/supported-products/{product_id}?error=No mirrors selected for deletion",
            status_code=302,
        )
    ...
    try:
        mirror_id_list = [
            int(mid.strip()) for mid in mirror_ids.split(",") if mid.strip()
        ]
    except ValueError:
        return RedirectResponse(
            f"/admin/supported-products/{product_id}?error=Invalid mirror IDs provided",
            status_code=302,
        )

    if not mirror_id_list:
        return RedirectResponse(
            f"/admin/supported-products/{product_id}?error=No valid mirror IDs provided",
            status_code=302,
        )
    ...
    if not mirrors:
        return RedirectResponse(
            f"/admin/supported-products/{product_id}?error=No mirrors found with provided IDs",
            status_code=302,
        )

    # Check dependencies for each mirror
    blocked_mirrors = []
    for mirror in mirrors:
        blocks_count = await SupportedProductsRhBlock.filter(
            supported_products_rh_mirror=mirror
        ...
        if blocks_count > 0 or overrides_count > 0:
            ...
            blocked_mirrors.append(f"'{mirror.name}' ({' and '.join(error_parts)})")

    if blocked_mirrors:
        error_message = (
            f"Cannot delete {len(blocked_mirrors)} mirror(s) due to dependencies: "
            f"{', '.join(blocked_mirrors)}. Please remove dependencies first."
        )

        return RedirectResponse(
            f"/admin/supported-products/{product_id}?error={error_message}",
            status_code=302,
        )

    # Delete all mirrors (they have no dependencies)
    deleted_count = 0
    deleted_names = []
    for mirror in mirrors:
        deleted_names.append(mirror.name)
        await mirror.delete()
        deleted_count += 1

    success_message = (
        f"Successfully deleted {deleted_count} mirror(s): {', '.join(deleted_names)}"
    )
    return RedirectResponse(
        f"/admin/supported-products/{product_id}?success={success_message}",
        status_code=302,
    )
...
@router.get(
    "/{product_id}/mirrors/{mirror_id}/repomds/new", response_class=HTMLResponse
)
...
async def admin_supported_product_mirror_repomd_new_post(
    ...
    return RedirectResponse(
        f"/admin/supported-products/{product_id}/mirrors/{mirror_id}", status_code=302
    )


@router.get(
    "/{product_id}/mirrors/{mirror_id}/repomds/{repomd_id}", response_class=HTMLResponse
)
async def admin_supported_product_mirror_repomd(
    request: Request, product_id: int, mirror_id: int, repomd_id: int
):
    repomd = await SupportedProductsRpmRepomd.get_or_none(
        ...
        "supported_products_rh_mirror",
        "supported_products_rh_mirror__supported_product",
    )
...
@router.post(
    "/{product_id}/mirrors/{mirror_id}/repomds/{repomd_id}/delete",
...
async def admin_supported_product_mirror_block_new(
    request: Request, product_id: int, mirror_id: int, search: str = None
):
    mirror = await SupportedProductsRhMirror.get_or_none(
        id=mirror_id, supported_product_id=product_id
    ).prefetch_related("supported_product")
...
@router.post(
    "/{product_id}/mirrors/{mirror_id}/blocks/new", response_class=HTMLResponse
)
...
@router.post(
    "/{product_id}/mirrors/{mirror_id}/blocks/{block_id}/delete",
    response_class=HTMLResponse,
)
...
@router.get(
    "/{product_id}/mirrors/{mirror_id}/overrides/new", response_class=HTMLResponse
)
...
@router.post(
    "/{product_id}/mirrors/{mirror_id}/overrides/{override_id}/delete",
    response_class=HTMLResponse,
)
async def admin_supported_product_mirror_override_delete(
    request: Request, product_id: int, mirror_id: int, override_id: int
):
...
async def export_mirror_config(product_id: int, mirror_id: int):
    ...
    mirror = await SupportedProductsRhMirror.get_or_none(
        id=mirror_id, supported_product_id=product_id
    ).prefetch_related("supported_product", "rpm_repomds")
